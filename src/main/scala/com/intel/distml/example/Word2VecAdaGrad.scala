...
import com.intel.distml.util.scala.{FloatMatrixAdapGradWithIntKey, FloatMatrixWithIntKey}
import com.intel.distml.util.store.{FloatMatrixStoreAdaGrad, FloatMatrixStore}
...
//    var alpha : Float = 0f
  ...
  def run(p : Params): Unit = {
    ...
    val m = new Model() {
      registerMatrix("syn0", new FloatMatrixAdapGradWithIntKey(wordTree.vocabSize, p.vectorSize))
      registerMatrix("syn1", new FloatMatrixAdapGradWithIntKey(wordTree.vocabSize, p.vectorSize))
    }
    ...
    dm.random("syn0")
    dm.zero("syn1")
    dm.setAlpha("syn0", initialAlpha, initialAlpha * 0.0001f, p.alphaFactor.toFloat)
    dm.setAlpha("syn1", initialAlpha, initialAlpha * 0.0001f, p.alphaFactor.toFloat)
        ...
        while (it.hasNext) {
          ...
          for (wIds : Array[Int] <- samples) {
            ...
            for (sentence_pos <- 0 to sentence.size - 1) {
              ...
              if (p.cbow) {
                cbow(wordTree, sentence, sentence_pos, p.window, b, syn0s, syn1s, alpha0s, alpha1s, neu1, neu1e, expTable)
              }
              else {
                skipGram(wordTree, sentence, sentence_pos, p.window, b, syn0s, syn1s, alpha0s, alpha1s, neu1, neu1e, expTable)
              }
            }
          }
          trainClock.stop()
  ...
  : Iterator[(Int, Array[Float])] = {
    ...
    val store = stores.get("syn0").asInstanceOf[FloatMatrixStoreAdaGrad]
