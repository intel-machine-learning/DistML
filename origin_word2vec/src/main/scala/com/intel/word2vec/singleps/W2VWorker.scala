...
//      neuronArray(a).initVec(r)
  ...
  def partitionTraining(expTableBroadcast : Broadcast[Array[Float]],
                        wordMapBroadcast : Broadcast[mutable.HashMap[String, Int]],
                        wordTreeBroadcast : Broadcast[Array[NeuronNode]],
                        totalWords : Long,
                        outputFolder : String,
                        paramServer : String,
                        pushStep : Int,
    ...
    val w = new Worker(index, globalExpTable, wordTree, wordMap, totalWords, lines,
      startingAlpha, outputFolder, paramServer, pushStep, fetchStep)
    w.trainWords(index, lines)
  ...
  def main(args: Array[String]) {
    ...
    var paramSevice : ParamService = null

    paramSevice = new ParamService(outputFolder, wordTree, totalWords, startingAlpha)
    paramSevice.initModel()
    paramSevice.start()

    var dummy = lines.mapPartitionsWithIndex(
      partitionTraining(expTableBroadcast, wordMapBroadcast, wordTreeBroadcast,
        totalWords, outputFolder, paramServer, pushStep, fetchStep))
    dummy.saveAsTextFile(outputFolder + "/result")

    paramSevice.stopWork()
